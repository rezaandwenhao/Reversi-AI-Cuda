...
// Think ahead 3 steps
__global__ void get_mediumAI_move(char* board, int* playable_direction, int *p_row, int *p_column, int current_player, struct move* moves)
{
...
void make_next_move( )
{
    ...
    cudaMallocManaged((char**)&devBoard, 1 * 1 * 1 * 1 * 8 * 8 * sizeof(char));
	cudaMallocManaged((int**)&devplayDir, 8*8*8 * sizeof(int));
    cudaMallocManaged((struct move**)&moves, 64 * sizeof(struct move));
    get_mediumAI_move<<<1, 64>>>(devBoard, devplayDir, &row, &column, current_player, moves);
    max = moves[0].max;
    for(int i = 0; i < 64; i++)
    {
        if(moves[i].max > max)
